...
| `dogs` | Array of `EnemyPlacement` ({ x, y }) — dogs spawn at scene start but hold their Dormant pose until the seagull comes within `DOG_ACTIVATION_RANGE` (see `Dog.ts`) |
| `cats` | Array of `EnemyPlacement` ({ x, y }) — cats spawn at scene start but hold their Dormant pose until the seagull comes within `CAT_ACTIVATION_RANGE` (see `Cat.ts`) |
| `men` | Array of `EnemyPlacement` ({ x, y }) — currently a single end-of-level boss who spawns at scene start but holds his Dormant pose until the seagull comes within `MAN_ACTIVATION_RANGE` (see `Man.ts`) |
...
| `gameOverSound` | Optional `SoundAsset` played once by `GameScene.triggerGameOver()` after `stopAll()` silences the music |
...
A `Dog` is the first enemy character. It extends `Phaser.Physics.Arcade.Sprite` directly (not `Character` — the `CharacterState` machinery would be overkill for the simple Dormant ↔ Walking machine the dog uses). The dog walks left at a constant speed once activated, has gravity so it rests on the surface, and is dangerous to the seagull on contact: a head-on or below collision restarts the scene, but if the seagull lands on it from above (a "stomp") the dog dies via `die()`.

Module-level constants: `DOG_SCALE` (0.4), `DOG_GRAVITY` (600), `DOG_MAX_FALL_SPEED` (500), `DOG_WALK_SPEED` (80), `DOG_STOMP_POINTS` (50), `DOG_ACTIVATION_RANGE` (500 — matches `MAN_ACTIVATION_RANGE`). The walking sprite is loaded from `assets/enemies/dog-walking.png` — 8 frames at 168×180 each, played at 10fps on a loop.

**State machine (Dormant → Walking).** A module-scoped `DogState` const object defines the two states; the instance field `dogState` (named to avoid shadowing Phaser's `GameObject.state`) tracks the current one. The dog is constructed at scene start in `Dormant`: zero horizontal velocity, no animation playing (the texture shows walk-sheet frame 0 statically). `update()` tracks the seagull's horizontal distance and calls `activateIfDormant()` once it falls below `DOG_ACTIVATION_RANGE`, which switches state to `Walking`, plays the loop, and sets leftward velocity. This lets level designers place dogs anywhere in the world without them all marching the moment the level starts.
...
**`constructor(scene, x, y, seagull)`**
Creates the sprite, stores the seagull reference (for the activation proximity check in `update()`), registers it with the scene and physics world, applies scale/gravity/terminal velocity. Horizontal velocity stays at 0 — the dog only starts moving when activation fires.

**`registerAnimations()`**
Registers the walk animation with the scene. Does not auto-play — `activateIfDormant()` plays the loop when the dog wakes.
...
**`update()`**
While Dormant, checks horizontal distance to the seagull each frame and calls `activateIfDormant()` when within range. No-op once Walking.

---

## `src/objects/Cat.ts`

A `Cat` is the second enemy character. Same shape as `Dog` — extends `Phaser.Physics.Arcade.Sprite` directly, has gravity, dies when stomped from above, and uses the same Dormant → Walking state machine to delay activation until the seagull is nearby. Module-level constants mirror the dog (`CAT_SCALE` 0.4, `CAT_GRAVITY` 600, `CAT_MAX_FALL_SPEED` 500, `CAT_WALK_SPEED` 80, `CAT_STOMP_POINTS` 50, `CAT_ACTIVATION_RANGE` 500). The walking sprite is loaded from `assets/enemies/cat-walking.png` — 8 frames at 136×121 each, played at 10fps on a loop.

API matches `Dog` exactly: `static preload(scene)`, `constructor(scene, x, y, seagull)`, `registerAnimations()`, `stomp()` returning `{ killed: true, points: CAT_STOMP_POINTS }`, `die()`, and an `update()` that runs the Dormant proximity check. `GameScene` treats dogs, cats, and men uniformly through a single `enemies: Array<Dog | Cat | Man>` field, calling `enemy.stomp()` polymorphically rather than branching on the concrete class.

---

## `src/objects/Man.ts`

A `Man` is the third enemy character. Same shape as `Dog` and `Cat` — extends `Phaser.Physics.Arcade.Sprite` directly, walks left at a constant speed, has gravity. Module-level constants: `MAN_SCALE` (0.7), `MAN_GRAVITY` (600), `MAN_MAX_FALL_SPEED` (500), `MAN_WALK_SPEED` (80), `MAN_HITS_TO_KILL` (3), `MAN_HIT_FLASH_MS` (150), `MAN_STOMP_POINTS` (100), `MAN_ALERT_DELAY_MS` (2000). Two sprites are loaded: `assets/enemies/man-walking.png` (4 frames at 139×222, 10fps loop, animation key `man-walk`) and `assets/enemies/man-throwing.png` (4 frames at 217×222, 8fps, one-shot — `repeat: 0` — animation key `man-throw`).

The Man encapsulates a multi-hit kill mechanic and its own drop choice via `stomp()`:

- Non-fatal stomp (`hitsTaken < MAN_HITS_TO_KILL`): calls `playHitEffect()` (a brief red multiplicative tint via `setTint(0xff5555)` cleared after `MAN_HIT_FLASH_MS` by `scene.time.delayedCall`, guarded with `this.active` so a death tween mid-delay can't crash it), schedules an alert transition via `scheduleAlertIfIdle()`, and returns `{ killed: false, points: 0 }` — `GameScene` bounces the seagull but skips scoring and removal.
- Fatal stomp: returns `{ killed: true, points: MAN_STOMP_POINTS, drop: BREAD }` — `GameScene` awards points, drops a piece of bread at the man's position, and runs the death tween.
...
**State machine (Dormant → Walking ↔ Alert).** A module-scoped `ManState` const object defines the three states; the instance field `manState` (renamed from `state` to avoid shadowing Phaser's public `GameObject.state` field) tracks the current one. The state machine is fully encapsulated — `GameScene` is unchanged.

- Initial state is `Dormant`: zero horizontal velocity, no animation playing (the texture shows walk-sheet frame 0 statically). The man is constructed at scene start so that all the existing `enemies`-array colliders and overlaps work uniformly, but he holds this pose until provoked. `update()` tracks the seagull's horizontal distance and calls `activateIfDormant()` once it falls below `MAN_ACTIVATION_RANGE` (500 — slightly more than the half-viewport, so he wakes roughly when he first comes on-screen). A stomp from above also wakes him via the same helper, since being landed on counts as provocation.
- `activateIfDormant()` switches `manState` to `Walking`, calls `faceSeagull()`, plays the `man-walk` loop, and sets velocity from the resulting `flipX` so he walks toward the seagull from whichever side she approached.
- A non-fatal stomp arms a 2-second `delayedCall` (`alertTimer`) via `scheduleAlertIfIdle()`. The timer is only armed if state is `Walking` and no timer is already pending — subsequent stomps within the 2s window do **not** reset it; the alert still fires 2s after the *first* stomp. (Dormant is excluded by this check, but the stomp's earlier `activateIfDormant()` call has already promoted the man to Walking, so the alert is armed anyway.)
- When the timer fires, `enterAlert()` switches `manState` to `Alert`, zeroes horizontal velocity, faces the seagull, plays `man-throw` (one-shot), and registers a one-time `animationcomplete-man-throw` listener.
- During the alert, `update()` calls `faceSeagull()` every frame so the throw pose tracks the seagull if it moves. `faceSeagull()` sets `flipX` based on whether the seagull is to the man's right (the raw frames face left, so `flipX === true` means facing right).
- `enterAlert()` also schedules a separate `throwTimer` (`MAN_STONE_RELEASE_DELAY_MS = 250`, half the animation's 500ms) that calls `releaseStone()` mid-animation, so the stone leaves the hand on the forward-arm pose rather than after the follow-through. The earlier "release on animation complete" timing read as the stone teleporting out after the throw was already over.
- On animation complete, `exitAlert()` returns `manState` to `Walking`, restarts the `man-walk` loop, and sets velocity from the current `flipX`: `+MAN_WALK_SPEED` if facing right, `-MAN_WALK_SPEED` otherwise. The man therefore continues walking in whatever direction he was last facing the seagull, instead of always resuming leftward.
- The seagull reference is injected into the constructor (`new Man(scene, x, y, seagull)`) and `GameScene.spawnEnemies()` passes `this.player`. `spawnPlayer()` runs before `spawnEnemies()` so `this.player` is always defined at construction time.
- `cancelPendingTransitions()` removes both `alertTimer` and `throwTimer` and detaches the `animationcomplete-man-throw` listener, so a man killed mid-windup never reaches `releaseStone()` and no orphan stone is fired.

**Stone-throw event.** `Man` exposes `onStoneThrow(handler)` — a thin typed wrapper over `this.on('man-throw-stone', handler)` — so `GameScene` doesn't need to know the magic event name. The handler receives `(spawnX, spawnY, seagullX)`. Spawn coordinates are computed from the man's position with `STONE_SPAWN_OFFSET_X` (40px, sign mirrored by `flipX`) and `STONE_SPAWN_OFFSET_Y` (30px upward) so the stone leaves from the throwing-arm side and torso height instead of the sprite centre.
- `cancelPendingTransitions()` removes any pending `alertTimer` and detaches the `animationcomplete-man-throw` listener; `die()` calls it before the fade tween so a death mid-pending doesn't leak a timer or callback onto a destroyed sprite.

Stone spawning, physics, and collision wiring all live in `GameScene.spawnStone()` rather than inside `Man` — the man only emits the throw event with the stone's spawn point and target.

---

## `src/objects/Stone.ts`

`Stone` is the projectile a `Man` throws while in his Alert state. Extends `Phaser.Physics.Arcade.Image` and uses a procedurally-generated circle texture (`stone-circle`, dark grey, 16×16) created in `Stone.preload(scene)` via `scene.make.graphics().generateTexture()` — `GameScene.preload()` calls it once per scene, and the `if (scene.textures.exists(...))` guard makes the call idempotent across scene restarts.

The constructor takes `(scene, x, y, targetX)`: it spawns at `(x, y)`, sets a circular Arcade body of radius 8 (so the visual hitbox matches the rendered circle), enables gravity (`STONE_GRAVITY = 600`), and gives the body an initial horizontal velocity (`±STONE_HORIZONTAL_SPEED = 240`) toward `targetX` plus an upward lift (`STONE_VERTICAL_LIFT = 200`) — the combination produces a parabolic arc that reads as a thrown stone rather than a flat shot.

Each stone also schedules a `STONE_LIFETIME_MS = 5000` self-destruct via `scene.time.delayedCall`. Without it, stones that miss everything (e.g. fired off the right edge of the world) would persist for the rest of the level, holding their listeners alive.

**Wiring in `GameScene.spawnStone(x, y, targetX)`** (called from each man's `onStoneThrow` handler in `spawnEnemies()`):

- Constructs the stone.
- `physics.add.overlap(player, stone)` — on hit, `stone.destroy()` then `triggerGameOver()`. The `gameOver` guard inside `spawnStone()` itself prevents new stones from being created after the player has died.
- `physics.add.collider(stone, surface | platforms)` — on landing, `stone.destroy()`. Without this the seagull could walk into a resting stone and trigger game-over long after the throw, which would feel like a delayed-action mine rather than a projectile.

Adding new projectile types is purely a matter of giving them their own class with the same `preload` + constructor shape; the spawn-and-wire flow in `GameScene` is the integration point.
...
Creates the sprite at `(x, y)` with `kind.textureKey`, stores the `kind` itself on a public `readonly kind: FoodKind` field (so callers can identify the food by reference rather than by texture-key string — `GameScene` uses this to detect the pizza pickup that ends the level), applies `kind.scale` if specified, copies `kind.pickupSound?.key` into `pickupSoundKey` and `kind.pickupSound?.volume` into `pickupSoundVolume`, registers the instance with the scene, and adds a static Arcade body so overlap detection works. A looping yoyo tween bobs the sprite ±6px vertically (800ms, `Sine.easeInOut`) to make pickups visually pop; the static body stays at the original `y`, so the small visual offset doesn't affect overlap detection.
...
When the seagull overlaps a `Food`, `GameScene` adds the points to `seagull.points`, plays `pickupSoundKey` if set, calls `spawnScorePopup` (from `src/ui/Hud.ts`) to show a `+N` floating text at the food's position, then calls `food.pickup()` to play the absorb animation. After the pickup, if `food.kind === PIZZA`, `GameScene.triggerLevelComplete()` fires — pizza is dropped only by the end-of-level man boss, so collecting one ends the level.

---

## `src/effects/Fireworks.ts`

Self-contained celebration effect used by `GameScene.triggerLevelComplete()`. Owns a procedurally-generated white circle texture (`firework-particle`, 6×6) created in `Fireworks.preload(scene)` via `scene.make.graphics().generateTexture()` — `GameScene.preload()` calls it once per scene, guarded with `scene.textures.exists(...)` so restarts are idempotent (mirrors `Stone.preload`).

**`Fireworks.start(scene)`** schedules `BURST_COUNT` (8) bursts spaced `BURST_INTERVAL_MS` (350ms) apart via `scene.time.delayedCall`. Each burst calls the private `spawnBurst(scene)`, which:

- Picks a random screen position in the camera viewport (camera-relative, using `cam.scrollX`/`cam.scrollY` so bursts always land on-screen even though the camera follows the seagull).
- Picks a random tint from a 6-colour palette (gold, pink, cyan, white, lime, orange).
- Creates a `Phaser.GameObjects.Particles.ParticleEmitter` with `emitting: false`, then calls `explode(PARTICLES_PER_BURST)` to fire one burst.
- Schedules a `delayedCall` to destroy the emitter after the particles' lifespan.

The emitter is placed at depth 10 so it renders above gameplay sprites but below the level-complete text (depth 20).
...
- **`preload`** — loads seagull assets (`Seagull.preload`), dog, cat and man enemy spritesheets (`Dog.preload`, `Cat.preload`, `Man.preload`), the surface texture for this level (`Surface.preload(this, level1Config.surface)`), the parallax layer images (`Background.preloadTextures`), one image per platform in `level1Config.platforms` (`Platform.preload`), one image per food kind referenced in `level1Config.foods` (`Food.preload`), the level's background music if specified, and the level's `gameOverSound` if specified.
- **`create`** — expands the physics world to the full level dimensions, instantiates the `Background`, `Surface`, and `Seagull` (the seagull is set to `depth 1` so it renders above the surface, platforms, enemies, and food, all of which stay at the default depth 0; the background uses negative depths and stays behind), registers a collider between the seagull and surface (the callback switches to `Walking` on contact), spawns platforms via `spawnPlatforms()` — one `Platform` per entry in `level1Config.platforms`, registered as a single collider against the player whose callback also transitions the seagull to `Walking` (so landing on a platform behaves identically to landing on the ground), spawns enemies via `spawnEnemies()` — one `Dog` per entry in `level1Config.dogs`, one `Cat` per entry in `level1Config.cats`, and one `Man` per entry in `level1Config.men`, all stored in a single `enemies: Array<Dog | Cat | Man>` field. All three enemy types receive `this.player` in their constructor so they can run their own Dormant proximity check; they are constructed at scene start but stay in their `Dormant` state (zero velocity, walk-sheet frame 0) until the seagull comes within range (see each enemy file) — and registers a seagull-vs-enemy overlap that branches on impact direction (a stomp from above — descending velocity and the seagull's bottom at/above the enemy's centre — calls `enemy.stomp()` to obtain a `StompOutcome` and applies it generically: `flap()` always, `points > 0` adds to the score and spawns a popup, `drop` spawns a `Food` at the enemy's position via `dropFood`, and `killed: true` removes the enemy from `this.enemies` and calls `enemy.die()` — no `instanceof` branches on enemy type; which spawns a `Food` and registers a single-target overlap whose callback delegates to `collectFood`; any other contact triggers the game-over flow via `triggerGameOver()`), spawns one `Food` per entry in `level1Config.foods` and registers an overlap whose callback also delegates to `collectFood` — a private helper that adds the food's points to `player.points`, plays the food's pickup sound at its configured volume, spawns a `+N` popup, and runs `food.pickup()` to fade and destroy the sprite — sets the camera to follow the player with a gentle lerp (`0.08`) within the level bounds, and starts the level's looping background music (if set) using the music's configured `volume`.

The scene tracks two terminal-state flags — `gameOver` (set by `triggerGameOver()`, fired on a fatal seagull collision or stone hit) and `levelComplete` (set by `triggerLevelComplete()`, fired from `collectFood` when the seagull picks up a `PIZZA`). While either is set, `update()` short-circuits so player input and enemy ticks are ignored. Because `scene.restart()` reuses the same scene instance (class-field initializers do not re-run), `create()` explicitly resets both flags to `false` and calls `physics.resume()` at the top — without this, the flags and the paused physics from the previous run would carry over and immediately freeze the new game.

**`triggerGameOver()` (private)**
Stops all sounds (so the looping background music doesn't keep playing under the fail screen), then plays `level1Config.gameOverSound` if set — order matters because `stopAll()` would otherwise silence the cue we just queued. Then freezes the player's arcade body, pauses physics, and runs a 700ms spin-and-fade tween on the seagull (`angle 0 → 360`, `alpha → 0.3`). On tween completion calls `showGameOverScreen()`. Idempotent — re-entry while already in game-over state is a no-op, so simultaneous overlaps with multiple dogs don't double-fire.
...
**`triggerLevelComplete()` (private)**
The win-state counterpart to `triggerGameOver`. Idempotent on `levelComplete`. Stops all sounds (silencing the background music), freezes the player's arcade body, pauses physics, kicks off `Fireworks.start(this)` (see `src/effects/Fireworks.ts`), and runs a 400ms `scaleX/scaleY` ×1.3 yoyo bounce on the seagull. On tween completion calls `showLevelCompleteScreen()`.

**`showLevelCompleteScreen()` (private)**
Adds three camera-locked text objects centred on the viewport: a large "LEVEL COMPLETE!" title, a `Score: N` line below it, and a "Press any key to restart" prompt — all styled from `BASE_HUD_TEXT_STYLE` and at depth 20 so they sit above the firework particles. Registers a one-shot `keydown` listener that restarts the scene, mirroring the game-over restart path.
- **`update`** — runs every frame. Space- or Up-just-pressed → `Flying` + flap. Then applies horizontal movement based on left/right keys. Then a fall-off check: if the seagull is in `Walking` or `Standing` and neither `body.touching.down` nor `body.blocked.down` is set (nothing is supporting it from below), it transitions back to `Flying` so gravity pulls it down — this is what makes walking off a platform edge actually fall. Then swaps between `Standing` and `Walking` symmetrically based on whether a movement key is held. Ticks every live enemy with `enemy.update()` (each enemy's own `update` runs its Dormant proximity check that wakes it when the seagull approaches), calls `player.clampToBounds()`, and drives parallax via `background.update(camera.scrollX)`.
